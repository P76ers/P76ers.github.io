...
/* harmony import */ var _default_language_355974c8_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(60368);
...
const DyteGrid = class {
  constructor(hostRef) {
    ...
    this.onViewModeChanged = ({ viewMode }) => {
      ...
      if (viewMode === 'PAGINATED') {
        ...
        // if active, add self
        this.participants = this.participants.concat(this.hideSelf ? [] : [this.meeting.self]);
      }
    };
  ...
  disconnectedCallback() {
    ...
    participants.removeListener('peerStartedPresenting', this.onPresentationStart);
    participants.removeListener('peerStoppedPresenting', this.onPresentationEnd);
  ...
  updateActiveParticipants() {
    ...
    this.hideSelf = self.webinarStageStatus !== 'ON_STAGE' || isRecorder;
    this.participants = [
      ...participants.active.toArray(),
      ...(participants.viewMode === 'ACTIVE_GRID' && !this.hideSelf ? [self] : []),
    ];
  ...
  updateWebinarStage() {
    if (this.meeting && this.meeting.meta.viewType === 'WEBINAR') {
      this.showWebinarStage = this.participants.length === 0;
    }
    else {
      this.showWebinarStage = false;
    }
  ...
  meetingChanged(meeting) {
    if (meeting != null) {
      ...
      participants.addListener('peerStartedPresenting', this.onPresentationStart);
      participants.addListener('peerStoppedPresenting', this.onPresentationEnd);
