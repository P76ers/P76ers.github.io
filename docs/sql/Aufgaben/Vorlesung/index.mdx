...
- Anzahl aller Artikel per Aggregatfunktion bestimmen
  ...
  FROM artikel;
  ```
- Anzahl aller Artikel per Aggregatfunktion bestimmen und Spaltennamen anpassen
  ```sql
  SELECT COUNT(*) as "Anzahl Artikel" 
  FROM artikel;
  ```
- Anzahl aller Artikel und Email-Adressen bestimmen, Kombination von Aggregatfunktionen
  ```sql
...
- Verknüpfung von Bedingungen (Alle Kunden aus Münster <u>**und**</u> Berlin)
  ...
  ```
- Sortieren nach 1. Land absteigend und 2. nach Firma
  ```sql
  SELECT Firma, Ort, Land
  FROM kunden 
...
- Umsätze pro Artikel anzeigen
  ```sql
  SELECT sum(Anzahl * Einzelpreis), Artikelname
...
- Bedingungen für Aggregatfunktion mit `HAVING`
  ...
  alternativ
  ```sql
  SELECT sum(Anzahl * Einzelpreis) Umsatz, 
      Artikelname
  FROM artikel, bestellungendetails 
  WHERE ArtikelID = ArtikelNR
  GROUP BY Artikelname
  ...
  ```
- Umsätze für alle Kunden, die mindestens 10000€ haben (Bedingung mit Aggregatfunktion)
  ```sql
  SELECT sum(Anzahl * Einzelpreis) Umsatz, 
      Firma
  FROM artikel, bestellungendetails, bestellungen, kunden 
